...
using UnityEditor;
    ...
    public class AINeedsSystem : MonoBehaviour
    {
        ...
        private void SearchItem(bool Health)
        {
            ...
            {
                AIItem Item = item.GetComponent<AIItem>();

                if (Item == null)
                    continue;

                if (Item.ItemCanBeFound == AIEnums.YesNo.Yes)
                {
            ...
            if (Items.Count > 1)
            {
                ...
                if (min != null)
                {
                    ...
                    Searching = true;
                    AISystem.AICommandManager.SetDestination(min.transform.position);
                }
            }
            else if(Items.Count == 1)
            {
                ...
                Searching = true;
                AISystem.AICommandManager.SetDestination(Items[0].transform.position);
            }
        }
    }

    #if UNITY_EDITOR
    [CustomEditor(typeof(AINeedsSystem))]
    public class AINeedsSystemEditor : Editor
    {
